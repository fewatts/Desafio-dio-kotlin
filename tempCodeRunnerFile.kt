...
class CustomOrder private constructor(
  ...
  class Builder {
    ...
    fun build(): CustomOrder {
        ...
        var tot = 0.00
        this.products.forEachIndexed { _ , product ->
            products.add(Product(product.name, product.price, product.quantity))
            tot += (product.price * product.quantity) 
          }
      return CustomOrder(this.customerName, products, tot, this.deliveryAddress)
    }
  }
